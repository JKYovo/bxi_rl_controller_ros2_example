...
def _service_name(value: str) -> str:
    name = value.strip()
    if not name or any(
        character not in _SERVICE_NAME_CHARS for character in name
    ):
        raise ValueError(
...
class HoloRetargetSourceService:
    """Manage one source service without stopping a pre-existing instance."""
    ...
    def __init__(self, service_name: str | None = None) -> None:
        ...
        if stable.returncode != 0:
            self.started_by_mod = False
            raise RuntimeError(
                f"HoloRetarget service {self.service_name!r} "
