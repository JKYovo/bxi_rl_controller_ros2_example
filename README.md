...
## 同型号 BXI 新机器部署（本仓库定制版本）

本仓库已经包含当前使用的控制代码、舞蹈模块、ONNX/NPZ 模型和遥控器映射。目标机器需要是同型号 BXI 机器人，并且已经安装 Ubuntu 22.04、ROS 2 Humble 和 BXI 基础环境 `/opt/bxi/bxi_ros2_pkg`。

先确认机器人硬件控制程序已经停止，不要在硬件节点运行时覆盖或构建：

```bash
pgrep -af '[b]xi_bms|[b]xi_example_bms|[e]xample_demo_hw.launch.py|[h]ardware_elf3|[b]xi_example_py_elf3_demo'
```

在新机器上克隆到控制服务使用的固定路径并构建：
...
构建完成只代表代码和资源已经安装，不会自动启动真机。首次上机前建议先核对 `install/share/bxi_example_py_elf3/mods/` 中的模块和模型，再按现场安全流程用遥控器启动。
