"""Ordered external reference_qpos receiver shared inside the RGMT Mod.
...
class ReferenceReceiver:
    ...
    def push_payload(
        ...
        *,
        ...
        if qpos.shape != (QPOS_DIM,):
            raise ValueError(
                f"reference_qpos must have shape ({QPOS_DIM},), "
                f"got {qpos.shape}"
            )
    ...
    def advance_after_successful_step(self, frame_index: int) -> None:
        with self._lock:
            ...
            keep_from = self._raw_window_indices(self._playout_frame)[0]
            while self._frames:
